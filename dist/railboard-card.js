class RailboardCard extends HTMLElement {
  ...
  static getStubConfig() {
    return {
      ...
      max_departures: 10,
      min_minutes: 0,  // New: Minimum minutes to show
    };
  }
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      min_minutes: config.min_minutes || 0,  // default 0
    };
  }
  ...
  renderCard(entity) {
    ...
    const tocAbbreviations = {
      'Southern': 'SN', 'Southeastern': 'SE', 'South Western Railway': 'SWR', 'Thameslink': 'TL',
      'Great Northern': 'GN', 'London Overground': 'LO', 'Gatwick Express': 'GX', 'Great Western Railway': 'GWR',
      'c2c': 'C2C', 'Greater Anglia': 'GA', 'Chiltern Railways': 'CH', 'CrossCountry': 'XC', 'East Midlands Railway': 'EMR',
      'LNER': 'LNER', 'Northern': 'NT', 'ScotRail': 'SR', 'TransPennine Express': 'TPE', 'Avanti West Coast': 'AW',
      'West Midlands Railway': 'WMR', 'Elizabeth line': 'EL',
    };
    ...
    if (departures.length === 0) {
      html += `<div style="text-align: center; padding: 60px 20px; color: var(--secondary-text-color);">
      ...
      for (let i = 0; i < maxDeps; i++) {
        ...
        if (dep.is_cancelled) { statusBg='#fee2e2'; statusColor='#991b1b'; statusText='CANCELLED'; }
        else if (dep.is_delayed) { statusBg='#fef3c7'; statusColor='#92400e'; statusText='+' + dep.delay_minutes + 'm'; }
...
html += `<div style="display: flex; align-items: center; gap: 8px; padding: 14px 12px; background: ${bgColor}; border-left: 5px solid ${tocColour}; margin-bottom: 4px; border-radius: 6px;">
...
class RailboardCardEditor extends HTMLElement {
  setConfig(config) {
    this._config = { ...config };
    if (!this._initialized) { this._initialized = true; this.render(); }
  }

  set hass(hass) { this._hass = hass; if (this._initialized) this.updateEntityPicker(); }
  ...
  render() {
    this.innerHTML = `
      <div class="card-config">
        <div class="entity-row"><label>Entity (Required)</label>
          <select id="entity-picker"><option value="">Select entity...</option></select></div>
        <div class="input-row"><label>Title (Optional)</label>
          <input type="text" id="title-input" value="${this._config.title || ''}" placeholder="e.g., Crystal Palace" /></div>
        <div class="input-row"><label>Max Departures</label>
          <input type="number" id="max-input" min="1" max="50" value="${this._config.max_departures || 10}" /></div>
        <div class="input-row"><label>Min Minutes</label>
          <input type="number" id="min-input" min="0" max="120" value="${this._config.min_minutes || 0}" /></div>
        <div class="switch-row"><label>Show Platforms</label>
          <input type="checkbox" id="platforms-switch" ${this._config.show_platforms !== false ? 'checked' : ''} /></div>
        <div class="switch-row"><label>Show Status</label>
          <input type="checkbox" id="status-switch" ${this._config.show_status !== false ? 'checked' : ''} /></div>
        <div class="switch-row"><label>Show Calling Points</label>
          <input type="checkbox" id="calling-switch" ${this._config.show_calling_points !== false ? 'checked' : ''} /></div>
        <div class="switch-row"><label>Show Operator Badge</label>
          <input type="checkbox" id="badge-switch" ${this._config.show_operator_badge !== false ? 'checked' : ''} /></div>
      </div>
      <style>.card-config { padding: 16px; } .entity-row, .input-row, .switch-row { display:flex; align-items:center; margin-bottom:12px; } label { flex:1; font-weight:500; color:var(--primary-text-color); } select,input[type="text"],input[type="number"] { flex:2; padding:8px; border:1px solid var(--divider-color); border-radius:4px; background:var(--card-background-color); color:var(--primary-text-color); } input[type="checkbox"] { width:40px;height:20px; cursor:pointer; }</style>`;
    this.setupListeners();
  ...
  setupListeners() {
    ...
    if (minInput) minInput.addEventListener('change', e => { this._config = {...this._config, min_minutes:parseInt(e.target.value)}; this.configChanged(this._config); });
